...
class Indicators(object):
    ...
    def calculate_aroon(self, df, length=14):
        """
    ...
    def calculate_aroon_vectorized(self, df, period=14):
        """
    ...
    def calculate_standard_deviation(self, df, timeframe=15):
        """
    ...
    def calculate_sma_slope_normalized(self, df, ma_period=50, slope_period=5, atr_period=14):
        """
    ...
    def calculate_spike(self, df, lookback = 25):
        """
    ...
    def calculate_consolidation(self, df, change=5):
        """
    ...
    def calculate_zero_lag_trend_signals(self, df, length=70, mult=1.2):
        """
        ...
        df['zlema'] = pta.ema(zlema_src, length=length)
        ...
        df['zlema_upper_band'] = df['zlema'] + volatility
        df['zlema_lower_band'] = df['zlema'] - volatility
        
        # 2. Trend Detection
        # Identify the exact bar where the price crosses the bands
        bull_cross = (df['close'] > df['zlema_upper_band']) & (df['close'].shift(1) <= df['zlema_upper_band'].shift(1))
        bear_cross = (df['close'] < df['zlema_lower_band']) & (df['close'].shift(1) >= df['zlema_lower_band'].shift(1))
        ...
        df['zlema_trend'] = pd.Series(trend_signal, index=df.index).ffill().fillna(0).astype(int)
        
        # 3. Signal Generation
        
        # Trend Reversal Signals (Big Arrows)
        # Bullish reversal: trend changes from -1 to 1
        bull_reversal = (df['zlema_trend'] == 1) & (df['zlema_trend'].shift(1) == -1)
        # Bearish reversal: trend changes from 1 to -1
        bear_reversal = (df['zlema_trend'] == -1) & (df['zlema_trend'].shift(1) == 1)
        
        df['zlema_trend_reversal'] = np.select([bull_reversal, bear_reversal], [1, -1], default=0).astype(int)
        
        # Entry Signals (Small Arrows)
        # Crossover/under of the zlema line itself
        bull_entry_cross = (df['close'] > df['zlema']) & (df['close'].shift(1) <= df['zlema'].shift(1))
        bear_entry_cross = (df['close'] < df['zlema']) & (df['close'].shift(1) >= df['zlema'].shift(1))
        
        # Condition: cross must happen while the trend is already established for two bars
        is_bull_trend_established = (df['zlema_trend'] == 1) & (df['zlema_trend'].shift(1) == 1)
        is_bear_trend_established = (df['zlema_trend'] == -1) & (df['zlema_trend'].shift(1) == -1)
        ...
        df['zlema_entry_signal'] = np.select([bull_entry_condition, bear_entry_condition], [1, -1], default=0).astype(int)
