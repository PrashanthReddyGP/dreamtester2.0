...
class CustomLabelingExecutor(BaseNodeExecutor):
    """Executor for 'customLabeling' node. Runs user-provided Python code."""
    
    def execute(self, node: Any, inputs: ExecutorInput, context: ExecutionContext) -> ExecutorOutput:
        print(f"Executing Custom Labeling Node: {node.data.get('label', node.id)}")
        ...
        sub_type = node.data.get('subType', 'labeling')
